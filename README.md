...
2. **Install dependencies** with your favorite .NET tool (`dotnet restore`)  
...
4. **Access Swagger UI**:  
Navigate to `http://localhost:<port>/swagger` to view and test endpoints
...
Models/
├── GeneticParameters.cs
├── TspRequest.cs

Services/
├── Interfaces/
│   └── ITspSolverService.cs
├── Implementations/
│   └── TspSolverService.cs
```
...
Let me know if you need help with Swagger setup, Dockerfile, or CI/CD integration!
